...
def generate_left_right_fist_feet_dataset_from_saved_data(base_route, subject_id_list, start_ts, end_ts, window_ts, overlap_ts,
                                                use_imagery, eeg_2d_map, map_2d_shape):
...
def generate_fist_feet_dataset_from_saved_data(base_route, subject_id_list, start_ts, end_ts, window_ts, overlap_ts,
                                                use_imagery, eeg_2d_map, map_2d_shape):
    """
    Only use fist and feet data from MMIDB dataset. 0 is fist, 1 is feet
...
def generate_left_feet_classes_dataset_from_saved_data(base_route, subject_id_list, start_ts, end_ts, window_ts, overlap_ts,
                                                use_imagery, eeg_2d_map, map_2d_shape):
    """
    Dataset for Hand (L) and both feet
...
def generate_right_feet_classes_dataset_from_saved_data(base_route, subject_id_list, start_ts, end_ts, window_ts, overlap_ts,
                                                use_imagery, eeg_2d_map, map_2d_shape):
    """
    Dataset for Hand (R) and both feet
...
def generate_left_right_feet_dataset_from_saved_data(base_route, subject_id_list, start_ts, end_ts, window_ts, overlap_ts,
                                                use_imagery, eeg_2d_map, map_2d_shape):
    ...
    for subject_id in subject_id_list:
        ...
        for task in task_list:
            ...
            epoch_label_list = epoch_label_list - 2
            epoch_data_list = epoch_data_list[epoch_label_list >= 0, :, :, :] # get left and right hand data
            epoch_ts_list = epoch_ts_list[epoch_label_list >= 0, :]
            epoch_label_list = epoch_label_list[epoch_label_list >= 0] 
            if task in group_2_task_list:
                epoch_label_list = epoch_label_list + 2
                epoch_data_list = epoch_data_list[epoch_label_list > 2, :, :, :] # get only feet, exclude fist data
                epoch_ts_list = epoch_ts_list[epoch_label_list > 2, :]
                epoch_label_list = epoch_label_list[epoch_label_list > 2]
                epoch_label_list = epoch_label_list - 1 # make the feet label as 2
            mmidb_full_epoch_data.append(epoch_data_list)
            mmidb_full_label.extend(epoch_label_list.tolist())
            mmidb_full_epoch_ts.append(epoch_ts_list)
            mmidb_full_subjects.extend([subject_id for _ in epoch_label_list])
...
def generate_left_right_rest_feet_dataset_from_saved_data(base_route, subject_id_list, start_ts, end_ts, window_ts, overlap_ts,
                                                use_imagery, eeg_2d_map, map_2d_shape):
...
def generate_rest_feet_dataset_from_saved_data(base_route, subject_id_list, start_ts, end_ts, window_ts, overlap_ts,
                                                use_imagery, eeg_2d_map, map_2d_shape):
    ...
    if use_imagery:
        ...
        group_2_task_list = [5, 9, 13]
    group_0_task_list = [1]
    task_list = group_0_task_list + group_2_task_list
    for subject_id in subject_id_list:
        subject_str = 'S%03d' % subject_id
        for task in task_list:
            task_str = subject_str + 'R%02d' % (task)
...
def train_validate_split_pretraining(dataset, val_ratio=0.2, seed=None):
    
    rng = np.random.default_rng(seed)

    all_indices = [i for i, _ in enumerate(dataset.epoch_subjects)]
    
    pretrain_val_indices, pretrain_indices = [], []

    for cls in np.unique(dataset.label[all_indices]):
        cls_indices = [i for i in all_indices if dataset.label[i] == cls]
        rng.shuffle(cls_indices)

        n_val = int(len(cls_indices) * val_ratio) 
        pretrain_val_indices.extend(cls_indices[:n_val])
        pretrain_indices.extend(cls_indices[n_val:])

    # Shuffle the final lists (optional but good for randomness)
    rng.shuffle(pretrain_indices)
    rng.shuffle(pretrain_val_indices)

    return pretrain_indices, pretrain_val_indices

def train_validate_split_5_fold_CV(dataset):
    """
    Perform 5-fold cross-validation splitting based on subject IDs.
    Each fold uses distinct subjects for validation.
    
    :param dataset: dataset object with attributes:
                    - dataset.epoch_subjects: list/array of subject IDs per sample
                    - dataset.label: labels (not used for splitting)
    :return: list of 5 tuples (train_indices, val_indices)
    """
    # Get unique subject IDs
    all_subjects = np.unique(dataset.epoch_subjects)
    print(f'subject ID: {all_subjects}')
    # Split subjects into 5 folds
    subject_folds = np.array_split(all_subjects, 5)

    folds = []
    for fold in range(5):
        val_subjects = subject_folds[fold]
        print(f'Validation subjects {fold}: {val_subjects}')
        train_subjects = np.concatenate([subject_folds[i] for i in range(5) if i != fold])

        # Get item indices belonging to these subjects
        train_indices = [
            idx for idx, subj in enumerate(dataset.epoch_subjects) if subj in train_subjects
        ]
        val_indices = [
            idx for idx, subj in enumerate(dataset.epoch_subjects) if subj in val_subjects
        ]
...
def train_validate_split_4_fold_CV_transfer_learning(dataset, target_subject, seed=None):

    rng = np.random.default_rng(seed)

    # All indices for target subject
    target_indices = [i for i, subj in enumerate(dataset.epoch_subjects) if subj == target_subject]



    # Prepare per-class indices
    class_indices = {}
    for cls in np.unique([dataset.label[i] for i in target_indices]):
        cls_idx = np.array([i for i in target_indices if dataset.label[i] == cls])
        rng.shuffle(cls_idx)
        class_indices[cls] = np.array_split(cls_idx, 4)  # split into 3 folds
    ...
    for fold in range(4):
        val_indices = []
        train_indices = []
        for cls, splits in class_indices.items():
            val_indices.extend(splits[fold])
            # Training indices = all other splits
            train_indices.extend(np.concatenate([splits[i] for i in range(4) if i != fold]))
        folds.append((train_indices, val_indices))
...
def sample_single_class_indices(mmidb_dataset, item_indices, class_label, sample_ratio, seed):
    """
    ...
    for idx in item_indices:
        if mmidb_dataset.label[idx] == class_label:
            tmp_sample = random.random()
            # Keep it if tmp_sample is <= sample_ratio
            if tmp_sample <= sample_ratio:
                sample_item_indices.append(idx)
        else:
            # Keep all items that don't match class_label
            sample_item_indices.append(idx)
    return sample_item_indices

if __name__ == "__main__":
    route = '../data/physionet.org/files/eegmmidb/1.0.0/'
