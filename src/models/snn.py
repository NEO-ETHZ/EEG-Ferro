...
class CUBASpikingCNN(nn.Module):
    ...
    def forward(self, input_data, states):
        ...
        mask_tc = Bernoulli(
            torch.full_like(torch.zeros(batch_size, 256, device=torch.device("cuda" if torch.cuda.is_available() else "cpu")), 1 - dropout_tc)).sample() / (
                       1 - dropout_tc)

        mask = Bernoulli(
            torch.full_like(torch.zeros(batch_size, 256, device=torch.device("cuda" if torch.cuda.is_available() else "cpu")), 1 - dropout_rec)).sample() / (
                       1 - dropout_rec)

        mask_fc = Bernoulli(
            torch.full_like(torch.zeros(batch_size, 256, device=torch.device("cuda" if torch.cuda.is_available() else "cpu")), 1 - dropout_fc)).sample() / (
                       1 - dropout_fc)
